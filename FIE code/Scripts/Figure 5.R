...
f5a <- ggplot(data)+
  ...
  geom_smooth(aes(x=FishingPressure,y=H_age_abundance),method = "lm",color="black",fill="black",alpha=0.1)+
  labs(title="(a) H of spawning stock vs. fishing mortality\nNEAC (1946-2020)")+
...
f5b <- ggplot(data)+
  geom_text(aes(x=SSB,y=H_age_abundance,label=str_sub(Year,3,4)),family="Calibri",show.legend = F)+
...
# 5 Combine figures -------------------------------------------------------
figure <- ggarrange(f5a,f5b,nrow=2)
ggsave("Figures/Figure 5.pdf",device = cairo_pdf,width = 6,height = 8)
