...
#read data
data <- read_excel("Data/A50_NEAC.xlsx") %>% 
  select(1,2,3)
...
f2a <- ggplot(data_regime_A50)+
  ...
  geom_text(data=regime_shift_year_A50,aes(x=mean,y=-Inf,label=mean),vjust=-1,family="Calibri",fontface=4)+
...
f2b <- ggplot(data_regime_L50)+
  ...
  geom_line(aes(x=Year,y=regime_mean),color="black",linetype="dashed")+
  geom_text(data=regime_shift_year_L50,aes(x=mean,y=-Inf,label=mean),vjust=-1,family="Calibri",fontface=4)+
